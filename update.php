 ...
 */
function db_add_column(&$ret, $table, $column, $type, $attributes = [])
{
 ...
 *   nothing, but modifies $ret parameter.
 */
function db_change_column(&$ret, $table, $column, $column_new, $type, $attributes = [])
{
    if (array_key_exists('not null', $attributes) and $attributes['not null']) {
        $not_null = 'NOT NULL';
    }
    ...
    foreach ($_POST['start'] as $module => $version) {
        ...
        if ($version <= $max_version) {
            foreach ($updates as $update) {
                if ($update >= $version) {
                    $operations[] = ['update_do_one', [$module, $update]];
                }
            }
        }
    }
...
function update_create_batch_table()
{
  ...
  // If batch table exists, update is not necessary
    ...
    $schema['batch'] = [
        'fields' => [
            'bid'       => ['type' => 'serial', 'unsigned' => true, 'not null' => true],
            'token'     => ['type' => 'varchar', 'length' => 64, 'not null' => true],
            'timestamp' => ['type' => 'int', 'not null' => true],
            'batch'     => ['type' => 'text', 'not null' => false, 'size' => 'big'],
        ],
        'primary key' => ['bid'],
        'indexes'     => ['token' => ['token']],
    ];
...
function update_fix_d6_requirements()
{
    ...
    if (drupal_get_installed_schema_version('system') < 6000 && !variable_get('update_d6_requirements', false)) {
        $spec = ['type' => 'int', 'size' => 'small', 'default' => 0, 'not null' => true];
        ...
        db_add_field($ret, 'system', 'info', ['type' => 'text']);
        db_add_field($ret, 'system', 'owner', ['type' => 'varchar', 'length' => 255, 'not null' => true, 'default' => '']);
        if (db_table_exists('locales_target')) {
            db_add_field($ret, 'locales_target', 'language', ['type' => 'varchar', 'length' => 12, 'not null' => true, 'default' => '']);
        }
        if (db_table_exists('locales_source')) {
            db_add_field($ret, 'locales_source', 'textgroup', ['type' => 'varchar', 'length' => 255, 'not null' => true, 'default' => 'default']);
            db_add_field($ret, 'locales_source', 'version', ['type' => 'varchar', 'length' => 20, 'not null' => true, 'default' => 'none']);
        }
        ...
        $schema['cache_block'] = [
            'fields' => [
                'cid'        => ['type' => 'varchar', 'length' => 255, 'not null' => true, 'default' => ''],
                'data'       => ['type' => 'blob', 'not null' => false, 'size' => 'big'],
                'expire'     => ['type' => 'int', 'not null' => true, 'default' => 0],
                'created'    => ['type' => 'int', 'not null' => true, 'default' => 0],
                'headers'    => ['type' => 'text', 'not null' => false],
                'serialized' => ['type' => 'int', 'size' => 'small', 'not null' => true, 'default' => 0],
            ],
...
if (empty($op)) {
    ...
    if (!empty($messages['warning'])) {
        drupal_maintenance_theme();
        echo theme('update_page', '<form method="post" action="update.php?op=info"><input type="submit" value="Continue" /></form>', false);
        exit;
    }
...
if (isset($output) && $output) {
    ...
    echo theme('update_page', $output, !$progress_page);
}
