...
function install_main()
{
    ...
    if (!$task) {
        ...
        variable_set('install_profile_modules', array_diff($modules, ['system']));
    }
...
function install_change_settings($profile = 'default', $install_locale = '')
{
    ...
    echo theme('install_page', $output);
    exit;
}
...
function install_settings_form(&$form_state, $profile, $install_locale, $settings_file, $db_url, $db_type, $db_prefix, $db_user, $db_pass, $db_host, $db_port, $db_path)
{
    ...
    if (count($db_types) == 0) {
        $form['no_db_types'] = [
            '#value' => st('Your web server does not appear to support any common database types. Check with your hosting provider to see if they offer any databases that <a href="@drupal-databases">Drupal supports</a>.', ['@drupal-databases' => 'http://drupal.org/node/270#database']),
        ];
    } else {
        $form['basic_options'] = [
            '#type'        => 'fieldset',
            '#title'       => st('Basic options'),
            '#description' => '<p>'.st('To set up your @drupal database, enter the following information.', ['@drupal' => drupal_install_profile_name()]).'</p>',
        ];

        if (count($db_types) > 1) {
            $form['basic_options']['db_type'] = [
                '#type'          => 'radios',
                '#title'         => st('Database type'),
                '#required'      => true,
                '#options'       => $db_types,
                '#default_value' => ($db_type ? $db_type : current($db_types)),
                '#description'   => st('The type of database your @drupal data will be stored in.', ['@drupal' => drupal_install_profile_name()]),
            ];
            $db_path_description = st('The name of the database your @drupal data will be stored in. It must exist on your server before @drupal can be installed.', ['@drupal' => drupal_install_profile_name()]);
        } else {
            if (count($db_types) == 1) {
                ...
                $db_path_description = st('The name of the %db_type database your @drupal data will be stored in. It must exist on your server before @drupal can be installed.', ['%db_type' => $db_types[0], '@drupal' => drupal_install_profile_name()]);
            }
        }

        // Database name
        $form['basic_options']['db_path'] = [
            '#type'          => 'textfield',
        ...
        $form['advanced_options']['db_prefix'] = [
            '#type'          => 'textfield',
            '#title'         => st('Table prefix'),
            '#default_value' => $db_prefix,
            '#size'          => 45,
            '#description'   => st('If more than one application will be sharing this database, enter a table prefix such as %prefix for your @drupal site here.', ['@drupal' => drupal_install_profile_name(), '%prefix' => $prefix]),
        ];
...
function _install_settings_form_validate($db_prefix, $db_type, $db_user, $db_pass, $db_host, $db_port, $db_path, $settings_file, &$form_state, $form = null)
{
    ...
    if (!empty($db_prefix) && is_string($db_prefix) && !preg_match('/^[A-Za-z0-9_.]+$/', $db_prefix)) {
        form_set_error('db_prefix', st('The database table prefix you have entered, %db_prefix, is invalid. The table prefix can only contain alphanumeric characters, periods, or underscores.', ['%db_prefix' => $db_prefix]), 'error');
    }
    ...
    if (!in_array($db_type, $databases)) {
        form_set_error('db_type', st('In your %settings_file file you have configured @drupal to use a %db_type server, however your PHP installation currently does not support this database type.', ['%settings_file' => $settings_file, '@drupal' => drupal_install_profile_name(), '%db_type' => $db_type]));
    } else {
        // Verify
...
function install_select_profile()
{
    ...
    if (count($profiles) == 1) {
        ...
        echo theme('install_page', drupal_get_form('install_select_profile_form', $profiles));
        exit;
    }
}
...
function install_find_locales($profilename)
{
    ...
    array_unshift($locales, (object) ['name' => 'en']);
    ...
    if (count($locales) == 1) {
        if ($profilename == 'default') {
            ...
            if (!empty($_GET['localize'])) {
                ...
                $output .= '<ul><li>'.st('Determine if <a href="@translations" target="_blank">a translation of this Drupal version</a> is available in your language of choice. A translation is provided via a translation package; each translation package enables the display of a specific version of Drupal in a specific language. Not all languages are available for every version of Drupal.', ['@translations' => 'http://localize.drupal.org']).'</li>';
        ...
        drupal_set_title(st('Choose language'));
        echo theme('install_page', drupal_get_form('install_select_locale_form', $locales));
        exit;
    }
}
...
function install_select_locale_form(&$form_state, $locales)
{
    ...
    foreach ($locales as $locale) {
        ...
        if (isset($languages[$name])) {
            $name = $languages[$name][0].(isset($languages[$name][1]) ? ' '.st('(@language)', ['@language' => $languages[$name][1]]) : '');
        }
        $form['locale'][$locale->name] = [
            '#type'          => 'radio',
...
function install_no_profile_error()
{
    ...
    echo theme('install_page', '<p>'.st('We were unable to find any installer profiles. Installer profiles tell us what modules to enable and what schema to install in the database. A profile is necessary to continue with the installation process.').'</p>');
    exit;
}
...
function install_already_done_error()
{
    ...
    echo theme('install_page', st('<ul><li>To start over, you must empty your existing database.</li><li>To install to a different database, edit the appropriate <em>settings.php</em> file in the <em>sites</em> folder.</li><li>To upgrade an existing installation, proceed to the <a href="@base-url/update.php">update script</a>.</li><li>View your <a href="@base-url">existing site</a>.</li></ul>', ['@base-url' => $base_url]));
    exit;
}
...
function install_tasks($profile, $task)
{
    ...
    if ($task == 'profile-install') {
        ...
        $batch = [
            ...
            'title'         => st('Installing @drupal', ['@drupal' => drupal_install_profile_name()]),
    ...
    if ($task == 'configure') {
        ...
        if (!variable_get('site_name', false) && !variable_get('site_mail', false)) {
            ...
            if (!drupal_verify_install_file($settings_file, FILE_EXIST | FILE_READABLE | FILE_NOT_WRITABLE) || !drupal_verify_install_file($settings_dir, FILE_NOT_WRITABLE, 'dir')) {
                drupal_set_message(st('All necessary changes to %dir and %file have been made, so you should remove write permissions to them now in order to avoid security risks. If you are unsure how to do so, please consult the <a href="@handbook_url">on-line handbook</a>.', ['%dir' => $settings_dir, '%file' => $settings_file, '@handbook_url' => 'http://drupal.org/getting-started']), 'error');
            } else {
                drupal_set_message(st('All necessary changes to %dir and %file have been made. They have been set to read-only for security.', ['%dir' => $settings_dir, '%file' => $settings_file]));
            }
            ...
            drupal_add_js(['copyFieldValue' => ['edit-site-mail' => ['edit-account-mail']], 'cleanURL' => ['success' => st('Your server has been successfully tested to support this feature.'), 'failure' => st('Your system configuration does not currently support this feature. The <a href="http://drupal.org/node/15365">handbook page on Clean URLs</a> has additional troubleshooting information.'), 'testing' => st('Testing clean URLs...')]], 'setting');
            drupal_add_js('
// Global Killswitch
if (Drupal.jsEnabled) {
  $(document).ready(function() {
    ...
    if ($task == 'finished') {
        drupal_set_title(st('@drupal installation complete', ['@drupal' => drupal_install_profile_name()]));
        $messages = drupal_set_message();
        $output = '<p>'.st('Congratulations, @drupal has been successfully installed.', ['@drupal' => drupal_install_profile_name()]).'</p>';
        $output .= '<p>'.(isset($messages['error']) ? st('Please review the messages above before continuing on to <a href="@url">your new site</a>.', ['@url' => url('')]) : st('You may now visit <a href="@url">your new site</a>.', ['@url' => url('')])).'</p>';
        $task = 'done';
    }
    ...
    if (isset($output)) {
        echo theme('maintenance_page', $output);
    }
}
...
{
    return ['configure', 'profile-install', 'profile-install-batch', 'locale-initial-import', 'locale-initial-batch', 'profile-finished', 'locale-remaining-batch', 'finished', 'done'];
}
...
More details about installing Drupal are available in INSTALL.txt.', ['@drupal' => drupal_install_profile_name(), '%file' => $file, '%default_file' => $conf_path.'/default.settings.php', '@handbook_url' => 'http://drupal.org/server-permissions']), 'error');
        } elseif (!$writable) {
            drupal_set_message(st('The @drupal installer requires write permissions to %file during the installation process. If you are unsure how to grant file permissions, please consult the <a href="@handbook_url">on-line handbook</a>.', ['@drupal' => drupal_install_profile_name(), '%file' => $file, '@handbook_url' => 'http://drupal.org/server-permissions']), 'error');
        }
    }
        ...
        foreach ($requirements as $requirement) {
            if (isset($requirement['severity']) && $requirement['severity'] == REQUIREMENT_ERROR) {
                ...
                if (isset($requirement['value']) && $requirement['value']) {
                    $message .= ' ('.st('Currently using !item !version', ['!item' => $requirement['title'], '!version' => $requirement['value']]).')';
                }
...
function install_task_list($active = null)
{
    ...
    if (count($locales) < 2 || empty($_GET['locale']) || $_GET['locale'] == 'en') {
        ...
        $tasks += ['locale-remaining-batch' => st('Finish translations')];
    }
    ...
    if (in_array($active, ['finished', 'done'])) {
        $active = null;
    }
...
function install_configure_form(&$form_state, $url)
{
    ...
    $form['admin_account']['account']['name'] = ['#type' => 'textfield',
        '#title'                                         => st('Username'),
    ...
    $form['admin_account']['account']['mail'] = ['#type' => 'textfield',
    ...
    $form['server_settings']['date_default_timezone'] = [
        '#type'          => 'select',
    ...
    $form['server_settings']['clean_url'] = [
        '#type'          => 'radios',
        '#title'         => st('Clean URLs'),
        '#default_value' => 0,
        '#options'       => [0 => st('Disabled'), 1 => st('Enabled')],
    ...
    $form['server_settings']['update_status_module'] = [
        '#type'          => 'checkboxes',
        '#title'         => st('Update notifications'),
        '#options'       => [1 => st('Check for updates automatically')],
        '#default_value' => [1],
        '#description'   => st('With this option enabled, Drupal will notify you when new releases are available. This will significantly enhance your site\'s security and is <strong>highly recommended</strong>. This requires your site to periodically send anonymous information on its installed components to <a href="@drupal">drupal.org</a>. For more information please see the <a href="@update">update notification information</a>.', ['@drupal' => 'http://drupal.org', '@update' => 'http://drupal.org/handbook/modules/update']),
        '#weight'        => 15,
    ];
...
function install_configure_form_submit($form, &$form_state)
{
    ...
    $account = user_load(1);
    $merge_data = ['init' => $form_state['values']['mail'], 'roles' => [], 'status' => 1];
