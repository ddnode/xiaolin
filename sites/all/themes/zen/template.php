 ...
 */
function zen_links($links, $attributes = ['class' => 'links'], $heading = '')
{
    global $language;
    $output = '';
...
function zen_preprocess_region(&$vars, $hook)
{
    ...
    $vars['classes_array'] = ['region', 'region-'.str_replace('_', '-', $vars['region'])];
...
if (!function_exists('drupal_html_class')) {
    ...
    function drupal_html_class($class)
    {
        ...
        $class = strtr(drupal_strtolower($class), [' ' => '-', '_' => '-', '/' => '-', '[' => '-', ']' => '']);
...
if (!function_exists('drupal_html_id')) {
    ...
    {
        $id = strtr(drupal_strtolower($id), [' ' => '-', '_' => '-', '[' => '-', ']' => '']);
